...
function eliminarDuplicados(arr = []) {
    ...
    for( const elementos of arr) {
      if(!unicos.includes(elementos)){
        unicos.push(elementos)
      }
...
function esPalindromo(texto='' ) {
  let textoLimpio=texto.toLowerCase().trim().replaceAll(' ','') ;
  let textoVolteado=textoLimpio.split('').reverse().join('') ;
  if(textoVolteado===textoLimpio){
    return true;
  }else{
    return false
  }
...
function elementosEnComun(arr1 = [], arr2 = []) {
  ...
  for (let i=0 ; i< arr1.length ; 1++) {
    ...
    if(arr2.incluides(elemento) && !resultado.includes(elemento)){
...
function reemplazarPalabra(frase = '' , viejaPalabra='', nuevaPalabra='') {
  return frase.replaceAll(viejaPalabra, nuevaPalabra)
}
...
function caracterFrecuente(texto = '') {
  const limpieza = texto.replaceAll(' ','')
  
  const contar={};
  let maxRepetidas=0;
  let letraRepetida='';
  for(letras of limpieza)  {
    contar[letras] = (contar[letras] || 0) +1
    
    if ( contar[letras] > maxRepetidas){ 
      maxRepetidas=contar[letras]
...
function contarOcurrencias(frase = '') {
  ...
  const limpieza= frase.split(' ').filter(Boolean)

  const contar={};

  for(let palabra of limpieza ){
...
function obtenerLongitudes(texto = '') {
  separar= texto.split(' ')
  contar=[]
  for(let elemento of separar ){
    longitud=elemento.length
    contar.push(longitud)
  }
  return contar 
}
